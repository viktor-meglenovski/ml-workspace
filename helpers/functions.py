...
def save_intermediary_dataset(dataset: pd.DataFrame, working_directory_path: Path, dataset_type: Literal["dropped_columns", "missing_values_handled", "categorical_features_encoded", "continuous_features_scaled"]) -> None:
    dataset_name = f"dataset_{dataset_type}.csv"
    dataset_path = working_directory_path / INTERMEDIARY_DATASETS_FOLDER / dataset_name
    dataset.to_csv(dataset_path, index=False)
    logger.info(f"Dataset snapshot '{dataset_name}' saved.")
